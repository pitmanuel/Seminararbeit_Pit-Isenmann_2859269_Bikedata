...
for station in stations:
    ...
    df = data[data["Zählstation"] == station].copy()            #Filter Station
    start_date = pd.Timestamp(station_dates[station])           #Filter Startdatum
    ...
    seasonal_mean = df.groupby(['Wochentag', 'Stunde', "Monat"])['Wert'].mean().reset_index()
    seasonal_mean.rename(columns={'Wert':'Wert_seasonal'}, inplace=True)

    df = df.merge(seasonal_mean, on=['Wochentag', 'Stunde', "Monat"], how='left')
    y_true = df['Wert'].values
    y_pred = df['Wert_seasonal'].values
    ...
    print(f"Saisonaler Mittelwert R²: {r2:.3f}")
    print(f"Saisonaler Mittelwert RMSE: {rmse:.3f}")
    print(f"Saisonaler Mittelwert MAE: {mae:.3f}")
    print(f"Saisonaler Mittelwert CV: {cv:.3f}")
